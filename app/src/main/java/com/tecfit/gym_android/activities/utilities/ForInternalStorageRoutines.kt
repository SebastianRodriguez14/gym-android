...
import com.tecfit.gym_android.models.custom.RoutinesCounter
...
//                Toast.makeText(context, "Saved text to ${context.filesDir}", Toast.LENGTH_SHORT).show()
        ...
        private fun formatRoutinesExercisesToText(routinesExercisesInternalStorage: MutableList<RoutinesExercisesInternalStorage>):String{
            ...
            if (routinesExercisesInternalStorage.isEmpty()){
                ...
                for (reInternalStorage in routinesExercisesInternalStorage){
                    // sub_text -> "1+3:" (esto es un ejemplo)
                    var sub_text = "${reInternalStorage.id_routine}+${reInternalStorage.number_exercises}:"

                    for (id_exercises in reInternalStorage.id_exercises){
                        sub_text += "${id_exercises}-"
                    }
                    // sub_text -> "1+3:2-3-4" (esto es un ejemplo de como se vería el sub_text después de la siguiente línea)
                    sub_text = sub_text.substring(0, sub_text.length-1)

                    // text -> "1+3:2-3-4|" (un ejemplo de como se vería text)
                    text += "$sub_text|"

                }
                // text -> "1+3:2-3-4|2+3:1-2-4|3+2:1-2" (un ejemplo de como se vería text después de la siguiente línea)
        ...
        private fun formatTextToRoutinesAndExercises(text:String):MutableList<RoutinesExercisesInternalStorage>{
            ...
            // separate_routines -> [ "1+3:1-2-3", "2+3:2-3-4"] (ejemplo de como debería verse)
            val separate_routines = text.split("|")

            for (routine in separate_routines){
                // routine_exercise_detail -> [ "1+3", "1-2-3"] (ejemplo)
                val routine_exercise_detail = routine.split(":")
                // routine_detail -> [ "1", "3"]
                val routine_detail = routine_exercise_detail.get(0).split("+")
                val id_routine = routine_detail.get(0).toInt()
                val number_exercises = routine_detail.get(1).toInt()

                val id_exercises = mutableListOf<Int>()

                //id_exercises -> [ "1", "2", "3" ] (ejemplo)
                val id_exercises_text = routine_exercise_detail.get(1).split("-")

                for (id_exercise in id_exercises_text){
                    id_exercises.add(id_exercise.toInt())
                }

                val reInternalStorage = RoutinesExercisesInternalStorage(id_routine, number_exercises, id_exercises)
        ...
        /*
        * Para los contadores usaremos el siguiente formato en el fichero de texto
        * completed_routines|routines_started*/

        fun saveRoutineCounter(routinesCounter: RoutinesCounter, context: Context?){

            val text = formatRoutineCounterToText(routinesCounter)

            try {

                fileOutputStream = context!!.openFileOutput(FILE_COUNTERS, 0)
                fileOutputStream.write(text.toByteArray())
        ...
        private fun formatRoutineCounterToText(routinesCounter: RoutinesCounter):String{
            return "${routinesCounter.completed_routines}|${routinesCounter.routines_started}"
        }
